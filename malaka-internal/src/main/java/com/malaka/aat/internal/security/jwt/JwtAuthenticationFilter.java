package com.malaka.aat.internal.security.jwt;

import com.fasterxml.jackson.databind.ObjectMapper;
import com.malaka.aat.internal.security.CustomUserDetailsService;
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    private String getJwtFromRequest(HttpServletRequest request) {
        ...
        // Fallback: Get token from query parameter (for video streaming in <video> tag)
        // Only allow this for specific streaming endpoints
        String uri = request.getRequestURI();
        if (uri != null && uri.contains("/content/stream")) {
            String tokenParam = request.getParameter("token");
            if (StringUtils.hasText(tokenParam)) {
                log.debug("Token extracted from query parameter for streaming endpoint: {}", uri);
                return tokenParam;
            }
        }
